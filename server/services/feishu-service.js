 ...
 * 新设备创建通知卡片
 */
function buildDeviceCard(device, mentionOpenId) {
  const elements = [];
  const mentionLine = mentionOpenId ? `<at id="${mentionOpenId}"></at> 有新设备已录入，请确认版本信息\n` : '';
  elements.push({
    tag: 'div',
    text: {
      tag: 'lark_md',
...
async function sendIssueNotification(issue, assigneeOpenId) {
  try {
    ...
    await sendGroupMessage(chatId, card);
  } catch (err) {
    console.error('[飞书] 发送问题通知失败:', err.message);
  }
}

async function sendDeviceNotification(device, notifyOpenId) {
  try {
    const config = await getConfig();
    const chatId = resolveChatId(config);
    if (!config || !chatId) return;
    const card = buildDeviceCard(device, notifyOpenId);
    await sendGroupMessage(chatId, card);
  } catch (err) {
